...
export class Multiselect extends React.Component {
  constructor(props) {
    ...
    this.hideOptionList = this.hideOptionList.bind(this);
  ...
  renderXButton() {
    return (
      <div
        className="top-close-container"
        onClick={() => this.hideOptionList()}
      >
        <div className="top-close-container--button">+</div>
      </div>
    );
  }
        ...
        <ul className={`optionContainer`} style={style["optionContainer"]}>
          {typeof loadingMessage === "string" && (
            <span
              ...
              className={`notFound ${ms.notFound}`}
            >
              {loadingMessage}
            </span>
          )}
    ...
    return Object.keys(groupedObject).map((obj) => {
      return (
        <React.Fragment key={obj}>
          <li className={ms.groupHeading} style={style["groupHeading"]}>
            {obj}
          </li>
  ...
  renderMultiselectContainer() {
    ...
    return (
      <OutsideAlerter onOutsideClick={this.hideOptionList}>
        <div
          className={`${ms.multiSelectContainer} ${
            disable ? `${ms.disable_ms} disable_ms` : ""
          }`}
          id={id || "multiselectContainerReact"}
          style={style["multiselectContainer"]}
        >
          <div
            className={`${ms.searchWrapper} ${
              singleSelect ? ms.singleSelect : ""
            }`}
            ref={this.searchWrapper}
            style={style["searchBox"]}
            onClick={singleSelect ? this.showOptionList : () => {}}
          >
            ...
            <input
              ...
              value={inputValue}
              readOnly={!this.state.focusOnClick}
            ...
            />
            <div className="dropdown-triangle-multiselect" onClick={this.toggelOptionList} ></div>
            {singleSelect && (
              <i className={`icon_cancel ${ms.icon_down_dir}`} />
            )}
